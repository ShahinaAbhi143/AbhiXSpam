...
from AbhiExtra.get_user import user_only
from AbhiExtra.raid import RAID
...
async def uspam(AbhiXSpam: Client, e: Message):
    global unlimited
    unlimited = True
    msg = str(e.text[6:])
    if not msg:
...
async def uraid(AbhiXSpam: Client, e: Message):
    global unlimited
    unlimited = True
    user = await user_only(AbhiXSpam, e)
...
async def abuse(AbhiXSpam: Client, e: Message):
    global unlimited
    unlimited = True
    if e.reply_to_message:
        lmao = e.reply_to_message
...
async def echo_(AbhiXSpam: Client, message: Message):
    ...
    try:
        ...
        await AbhiXSpam.send_message(message.chat.id, msg)
    except Exception as a:
        await AbhiXSpam.send_message(message.chat.id, msg)
        print(str(a))
