...
from AbhiExtra.raid import RAID, RRAID
...
async def raid(AbhiXSpam: Client, e: Message):
    ...
    if len(lol) == 2:
        ...
        if not owo:
            ...
            return
        try:
            user = await AbhiXSpam.get_users(lol[1])
        except:
            await e.reply_text("**Error:** User not found!")
            return
    elif e.reply_to_message:
        try:
            user = await AbhiXSpam.get_users(e.reply_to_message.from_user.id)
        except:
...
async def rraid(AbhiXSpam: Client, e: Message):
    global USERS
    try:
    ...
    if e.reply_to_message and e.reply_to_message.from_user:
        ...
        try:
            user = await AbhiXSpam.get_users(user_)
...
async def draid(AbhiXSpam: Client, e: Message):
    global USERS
    try:
        lol = e.text.split(" ", 1)[1].split(" ", 1)
    except IndexError:
...
async def rlist(AbhiXSpam: Client, e: Message):
    global USERS
    _reply = "**Raid users list - Pb Bot Spam** \n\n"
    if len(USERS) > 0:
        for x in USERS:
            try:
                user = await AbhiXSpam.get_users(x)
...
async def checkraid(AbhiXSpam: Client, msg: Message):
    global USERS
