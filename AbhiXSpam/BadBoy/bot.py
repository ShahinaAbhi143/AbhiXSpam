...
from AbhiExtra.get_time import get_time
from pyrogram import Client, filters
from pyrogram.types import Message

from AbhiXSpam import start_time
...
async def restarter(AbhiXSpam: Client, message: Message):
    await message.reply_text(
        f"**Bot Is Restarting**\n\n Please Wait 5 min till bot is restart.\nAfter 5 Min Type {HANDLER}ping"
    )
    try:
        await AbhiXSpam.stop()
    except Exception as error:
        print(str(error))

    args = [sys.executable, "-m", "AbhiXSpam"]
